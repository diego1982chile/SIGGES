...
VALUES ( 
 'Diego Soto', 
 '(004) [AT.NCAT] DS MD NCAT_PCK_UNION.pkb', 
 'Creación de definición de package NCAT_PCK_UNION',  
  SYSDATE, 
  'SIGG-ME578'); 
...
BEGIN
    ...
    SIS.SIS_PCK_HELPER.RESETSEQ('NCAT.NCAT_TAB_RELAUNRE','NCAT.NCAT_SEQ_RELAUNRE');
     ...
     for c in ( select IB.PROBLSALUD probsalud, ib.COD_PS cod_ps, IB.RAMA rama, IB.FAMRAMA famrama, IB.COD_FAMILIA cod_familia, IB.PRESTACION prestacion, 
                       ib.ARANCEL arancel, ib.EDAD edad, ib.SEXO sexo, ib.FRECUENCIA frecuencia, ib.EXCLUYENTES excluyentes
                ...
                loop
                    ...
                    if(auge) then
                        AgregarUnionAuge(c.probsalud, c.cod_ps, c.rama, c.famrama, c.cod_familia, c.prestacion, c.arancel, c.edad, c.sexo, c.frecuencia, c.excluyentes);
                    else
...
BEGIN
    ...
    loop
        ...
        if(auge) then
            QuitarUnionAuge(c.probsalud, c.cod_ps, c.rama, c.famrama, c.cod_familia, c.prestacion, c.arancel, c.edad, c.sexo, c.frecuencia, c.excluyentes);
        else
  ...
  IS
    ...
    exception
        ...
        when union_existente then
            ...
            v_msgora  := '.';
            v_codlog  := 1;
            v_dsc_log := 'union existente para famrama de glosa '||dsc_famrama||', prestación de código '||cod_pres;
            raise;
...
PROCEDURE AgregarUnionAuge(
  ...
  IS
   ...
   v_pk_pres:= SIS.SIS_PCK_PRESTACION.EXISTE('prestacion',cod_pres);
    ...
    v_pk_union:=NCAT.NCAT_PCK_UNION.Existe('union_unincpre',cod_ps, v_pk_pres);

    if(v_pk_union is null) then
        select ncat.ncat_seq_union.nextval into v_pk_union from dual;
        insert into ncat.ncat_tab_union(UNION_COD_UNION, TIPTRAPA_COD_TIPTRAPA, DOCU_COD_DOCU, UNION_FEC_VIGE, UNION_FEC_VCTO, UNION_FEC_CREACION,
                                                     UNION_FEC_MODIFICACION, UNION_PGGENER_O_FAMRAMA)
        values (v_pk_union, tiptrapa, cod_docu, fecha_ini,  fecha_fin, fecha_ini, sysdate, cod_ps);
    else
        raise union_existente;
    end if;
...
PROCEDURE QuitarUnionAuge(
  ...
  IS
    ...
    loop
        -- TODO: Chequear que no exista regla para este cod_union y esta prestacion
        v_pk_unexcpre:=Existe('unexcpre',v_pk_union, c.param);
        if(v_pk_unexcpre is not null) then
            delete NCAT.NCAT_TAB_UNEXCPRE unex where unex.UNEXCPRE_COD_UNEXCPRE=v_pk_unexcpre;
        else
            raise unexcpre_inexistente;
